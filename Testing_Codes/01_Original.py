...
def test(imageset):
    totaltime=time.time()
    def test_part1(classifier):
        time1=time.time()
        pred1=classifier.predict(x_art)
        ...
        return[label1,'{0:.2f}'.format(confidence1),result1,time1]
    
    with open(f'{att}.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['INPUT_IMG','NORM_CLAS_ORIG_IMG','NCOI_CONF','NCOI_RSLT','NCOI_TIME','BET_CLAS_NO_RAND_OG_IMG','BCNROI_CONF','BCNROI_RSLT','BCNROI_TIME','BET_CLAS_OG_IMG','BCOI_CONF','BCOI_RSLT','BCOI_TIME'])
    for i,image_label in enumerate(imageset):
            x_art = np.expand_dims(imageset[i], axis=0)
            with open(f'{att}.csv', 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([image_labels[i]]+test_part1(normalclassifier)+test_part1(betterclassifierwithoutrandom)+test_part1(betterclassifier))
            print(i)
